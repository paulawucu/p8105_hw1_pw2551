...
penguins
```
* The variable names are: `r variable.names(penguins)`. Among these `r ncol(penguins)` attributes, "bill_length_mm", "bill_depth_mm", "flipper_length_mm", and "body_mass_g" look important and meaningful. The mean values for these variables will be calculated in r code chunks below. <br>
```{r, collapse = TRUE}
...
* The flipper length (mm) is mentioned above, which is `r round(flip_l, digits =2)`. <br>
